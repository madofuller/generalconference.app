  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="ml-0 lg:ml-[260px] min-h-screen flex-1 pt-20 lg:pt-0" style={{ background: '#fdf9e9' }}>
        <TopAppBar title="Paths of Service" subtitle="How leaders have been called to serve across the decades" />
        <div className="px-4 md:px-8 lg:px-12 pb-12 md:pb-24">
          {/* Headline */}
          <div className="bg-white p-6 rounded-xl shadow-[0px_12px_32px_rgba(27,94,123,0.06)] mb-4 md:mb-8">
            <p className="text-lg font-bold text-[#1c1c13]">{data.headline}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 lg:gap-8">
            ...
            <div className="md:col-span-7">
              {!selectedSpeaker ? (
                ...
                <div className="bg-white p-4 md:p-6 lg:p-8 rounded-xl shadow-[0px_12px_32px_rgba(27,94,123,0.06)] sticky top-24">
                  <h3 className="text-2xl font-extrabold text-[#1c1c13] mb-1">{selectedSpeaker.speaker}</h3>
                      ...
                      {selectedSpeaker.milestones.map((m, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-start gap-4 relative">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${getCallingColor(m.calling)}`}>
                              <span className="text-[10px] font-extrabold">{i + 1}</span>
                            </div>
                            <div className={`flex-1 p-4 rounded-xl ${isLast ? 'bg-[#f8f4e4] border-2 border-[#f5a623]/30' : 'bg-[#fdf9e9]'}`}>
                              <div className="flex items-center justify-between">
