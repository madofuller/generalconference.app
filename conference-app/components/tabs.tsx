...
export function PageTabs({ tabs, defaultTab, children }: PageTabsProps) {
  ...
  return (
    <>
      {/* Tab Bar */}
      <div className="z-20 bg-[#fdf9e9]/90 backdrop-blur-md border-b border-[#ece8d9] sm:sticky sm:top-20 lg:top-[64px]">
        <div className="relative overflow-hidden">
          ...
          <div
            ref={scrollRef}
            className="flex gap-0.5 sm:gap-1 overflow-x-auto scrollbar-hide px-2 sm:px-4 md:px-8 lg:px-12 py-2"
          >
            {tabs.map(tab => (
              <button
                key={tab.id}
                ...
                className={cn(
                  'flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-3 md:px-4 py-2.5 border-b-2 rounded-none text-[10px] md:text-[11px] font-bold uppercase tracking-wide sm:tracking-wider whitespace-nowrap transition-all shrink-0',
                  activeTab === tab.id
                    ? 'border-[#f5a623] text-[#1B5E7B] bg-[#f5a623]/10'
                    : 'border-transparent text-[#1c1c13]/30 hover:text-[#1B5E7B] hover:border-[#1c1c13]/10'
                )}
              >
                <span className="hidden sm:inline"><span className="material-symbols-outlined text-[14px] md:text-[16px]">{tab.icon}</span></span>
                {tab.label}
              </button>
            ))}
          </div>
